...
def create_fallback_champion_icon(target_path: str | Path, display_name: str) -> str:
    """Create a deterministic local placeholder when no provider has a hero portrait yet.

    The placeholder is intentionally generated locally so newly announced heroes never
    render as an empty slot.  A later real provider image replaces it normally.
    """
    ...
    if _valid_image(target):
        return str(target)
    try:
        image = Image.new("RGBA", (256, 256), (23, 31, 48, 255))
        draw = ImageDraw.Draw(image)
        draw.ellipse((24, 24, 232, 232), fill=(45, 63, 92, 255), outline=(214, 177, 92, 255), width=6)
        name = str(display_name or "?").strip()
        initials = "".join(part[:1] for part in re.findall(r"[A-Za-z0-9]+", name))[:2].upper() or "?"
        font = ImageFont.load_default()
        try:
            bbox = draw.textbbox((0, 0), initials, font=font, stroke_width=1)
            tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
        except Exception:
            tw, th = 12 * len(initials), 12
        # Scale the default bitmap glyphs to a large crisp monogram without a font dependency.
        glyph = Image.new("RGBA", (max(1, tw + 8), max(1, th + 8)), (0, 0, 0, 0))
        gd = ImageDraw.Draw(glyph)
        gd.text((4, 4), initials, font=font, fill=(245, 238, 214, 255), stroke_width=1, stroke_fill=(0, 0, 0, 255))
        glyph = glyph.resize((min(120, glyph.width * 8), min(120, glyph.height * 8)), Image.Resampling.NEAREST)
        image.alpha_composite(glyph, ((256 - glyph.width) // 2, (256 - glyph.height) // 2))
