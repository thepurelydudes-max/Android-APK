...
from localization import COMMON_CHAMPION_ALIASES, has_cyrillic, normalize_search, russian_hero_name, russian_item_name
from paths import APP_DIR

DB_PATH = APP_DIR / "mobilelegends.db"
...
def init_db() -> None:
    with connect() as con:
        ...
        _ensure_column(con, "items", "effect_ru", "TEXT NOT NULL DEFAULT ''")
        ...
        # a freshly upgraded offline database correct before the next web update.
        for row in con.execute("SELECT name,name_ru,effect_ru FROM items").fetchall():
            name_ru = russian_item_name(row["name"])
            if name_ru:
                con.execute("UPDATE items SET name_ru=? WHERE name=?", (name_ru, row["name"]))
            elif row["name_ru"] and not has_cyrillic(row["name_ru"]):
                con.execute("UPDATE items SET name_ru='' WHERE name=?", (row["name"],))
            if row["effect_ru"] and not has_cyrillic(row["effect_ru"]):
                con.execute("UPDATE items SET effect_ru='' WHERE name=?", (row["name"],))

        for row in con.execute("SELECT id,name,name_ru FROM champions").fetchall():
            fallback_ru = russian_hero_name(row["id"], row["name"])
            if fallback_ru and not has_cyrillic(row["name_ru"]):
                con.execute("UPDATE champions SET name_ru=? WHERE id=?", (fallback_ru, row["id"]))
...
def upsert_item_details(
    name: str, *, price: int = 0, stats: Iterable[str] = (), effect_en: str = "", effect_ru: str = "",
    ...
    with connect() as con:
        ...
        if row and data_hash and (row["data_hash"] or "") == data_hash:
            ...
            return False
        if row is None:
            con.execute("INSERT INTO items(name,source) VALUES(?,?)", (name, "mlbb"))
        con.execute(
            """UPDATE items SET price=?,stats_json=?,effect_en=?,
               effect_ru=CASE WHEN ?<>'' THEN ? ELSE effect_ru END,
               data_hash=?,data_patch=?,data_source_url=? WHERE name=?""",
            (int(price or 0), json.dumps(clean_stats, ensure_ascii=False), effect_en or "",
             effect_ru or "", effect_ru or "", data_hash or "", data_patch or "", source_url or "", name),
        )
    return True
...
def get_matchup_score(champion_id: str, enemy_id: str, preferred_role: str = "") -> float:
    with connect() as con:
        rows = con.execute(
            "SELECT role,score FROM matchups WHERE champion_id=? AND enemy_id=?",
